...
<section class="breadcrumb">
    <div class="container">
        ...
        @if($title_menu)
            <div class="right-align" itemscope="" itemtype="{{ route('home') }}">
                <a href="{{ Request::url() }}" itemprop="url"><span
                            itemprop="title"> {{ $title_menu }}</span></a>
            </div>
            <i class="right-align icon-angle-left"></i>
        @endif
        <div class="right-align divinhere" itemscope=""
             itemtype="{{ $_SERVER['SERVER_NAME'] . '/' . $_SERVER['REQUEST_URI'] }}">
            <span itemprop="title"><strong>{{ __("messages.Browse by Category") }}</strong></span>
        </div>
    </div>
</section>
<!-- breadcrumb ENDS -->
<section class=" listchar"><!--contentinnerwrapper-->
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-10 col-sm-12 col-md-10 col-xs-12 divmaincontent categorylist">
                <h2 class="title-cat titleorder2">{{ $page ?: __("messages.Browse by Category") }}</h2>
                <div class="row listcat">
                    @if ($categories)
                        @foreach ($categories as $i => $item)
                            ...
                            <div class="col-lg-4 col-sm-6 col-xs-12 col-md-6 item-cat">
                                <div class="cat_box_inr">
                                    ...
                                    >
                                        {{ app()->getLocale()=='fa'?$item->name_fa:$item->name }}
                                    </a>
                                    <div class="cat_pc">
                                        <figure>
                                            <a
                                                    ...
                                                    title="{{app()->getLocale()=='fa'?$item->name_fa:$item->name}}">
                                                <img
                                                        src="{{url('images/category/'.$item->image)}}"
                                                        width="173" height="80" alt=""></a></figure>
                                    </div>
                                    <ul class="cat_links">
                                        ...
                                        @if (!empty($item->descendants))
                                            ...
                                            <li>
                                                <a class="tooltip1"
                                                   href="{{route('home.categories',['slug'=>app()->getLocale()=='fa'?$subcategory->slug_fa:$subcategory->slug]) }}"
                                                   title="{{app()->getLocale()=='fa'?$subcategory->name_fa:$subcategory->name}}"
                                                >
                                                    {{app()->getLocale()=='fa'?$subcategory->name_fa:$subcategory->name}}
                                                                <span>
                                                                    <img
                                                                            src="{{url('images/category/'.$subcategory->image)}}"
                                                                            style="float: right">
                                                                </span>
                                                </a>
                                            </li>
                                        ...
                                        <li>
                                            <a
                                                    href="{{route('home.categories',['slug'=>app()->getLocale()=='fa'?$item->slug_fa:$item->slug]) }}"
                                                    class="act" title="">{{ __("messages.View All") }}</a></li>
                                    </ul>
                                </div>
                            </div>
                        @endforeach
